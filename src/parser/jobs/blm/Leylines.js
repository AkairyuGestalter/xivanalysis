...
export default class Leylines extends Module {
	...
	_onCast(event) {
		if (this._castingEvent) {
			this._castingEvent = null
		}
		if (this._leyLines.current) {
			if (!this._leyLines.current.stop) {
				this._leyLines.current.casts.push(event)
			}
		}
		if (this._circleOfPowers.current) {
			if (!this._circleOfPowers.current.stop) {
				this._circleOfPowers.current.casts.push(event)
			}
		}
	}
	...
	_onRemoveLeyLines(event) {
		if (this._leyLines.current) {
			...
			if (this._castingEvent) {
				this._leyLines.current.casts.push(this._castingEvent)
			}
		}
	...
	_onRemoveCircleOfPower(event) {
		if (this._circleOfPowers.current) {
			...
			if (this._castingEvent) {
				this._circleOfPowers.current.casts.push(this._castingEvent)
			}
		}
	...
	_onComplete(event) {
		...
		//filter out the last possible LL usage because it would make things weird.  - STOPPING THIS NOW
